					...
					<div class="col-lg-9">
						<div class="panel panel-default gradient">
							<div class="panel-heading">
								<h4>
									<span>Teacher Workload Fact Table</span>
								</h4>
							</div>
								...
								<table cellpadding="0" cellspacing="0" border="0"
									...
									<tbody>
										...
										<%
											...
											//String Hivequery = "SELECT t1.teacher_fact_id,t2.teacher_name,t3.course_name,t1.date_key,t1.do_workload FROM teacher_fact t1,teacher t2,course0 t3 WHERE t1.teacher_id = t2.teacher_id AND t1.course_id=t3.course_id";
											String Hivequery = "SELECT * FROM teacher_workload_fact";
											System.out.println("Running: " + Hivequery);
											ResultSet res = stmt.executeQuery(Hivequery);
											System.out.println("execute :" + res);
											while (res.next()) {
												
												String teacher_fact_id = res.getString(1);
												String teacher_id = res.getString(2);
												String course_id = res.getString(3);
												String date_key = res.getString(4);
												String do_workload = res.getString(5);
												String withdraw = res.getString(6);

												
												request.setAttribute("teacher_fact_id", teacher_fact_id);
												request.setAttribute("teacher_id", teacher_id);
												request.setAttribute("course_id", course_id);
												request.setAttribute("date_key", date_key);
												request.setAttribute("do_workload", do_workload);
												request.setAttribute("withdraw", withdraw);
										%>

										<tr>
												<td>${teacher_fact_id}</td>
												<td>${teacher_id}</td>
												<td>${course_id}</td>
												<td>${date_key}</td>
												<td>${do_workload}</td>
											...
											<td>
												<div class="controls center">
													<form action="delDataTeacherWorkloadFact" method="post">
														<button name="deleteteacher_fact_id" value="${teacher_fact_id}"
															title="Delete"
															onclick="res = confirm('Are you sure you want to detete Teacher_Fact_ID : ${teacher_fact_id} ?'); if(res){ LoadingFunction(); }return res;">
					...
					<div class="col-lg-3">
						...
						<div class="panel panel-default chart">
							...
							<div class="panel-heading">
								...
								<h4>
									<span class="icomoon-icon-pencil-3"></span> <span>Insert Data</span>
								</h4>
							...
							<div class="panel-body">
								...
								<div style="height: 450px; width: 100%;">
									...
									<b>Download Template : </b> <a
										href="files/TeacherWorkloadTemplate.csv">
											<button class="btn btn-default btn-xs">
												<span class="icomoon-icon-folder-download"></span>Workload
											</button></a>

									<form action="UpdateCSVTeacherWorkloadFact" method="post"
											...
											<center>
												...
												<c:choose>
													<c:when test="${AddisFail=='false'}">
														<h3 id="green">Add Degree Data Success!</h3>
													</c:when>
													<c:when test="${DelisFail=='false'}">
														<h3 id="green">Delete Degree Data Success!</h3>
													</c:when>
													<c:when test="${AddisFail=='true'}">
														<h3 id="red">Add Degree Data Fail, Please try again!</h3>
													</c:when>
													<c:when test="${DelisFail=='true'}">
														<h3 id="red">Delete Degree Data Fail, Please try again!</h3>
													</c:when>
													<c:otherwise>
